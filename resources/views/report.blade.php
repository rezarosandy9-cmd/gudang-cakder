...
<nav class="bg-orange-500 sticky top-0 z-50 shadow-lg border-b border-orange-400">
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 py-8">
        <div class="flex flex-col lg:flex-row justify-between items-end gap-6 mb-8">
            <h2 class="text-3xl md:text-5xl font-black text-gray-900 uppercase italic tracking-tighter">REKAPITULASI</h2>
            <a href="{{ route('report.export') }}" class="bg-green-600 text-white px-6 py-3 rounded-xl font-black uppercase italic text-[10px] shadow-lg">Export Excel</a>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left min-w-[1000px]">
                    ...
                    <tbody class="divide-y divide-gray-100 font-bold uppercase italic text-xs">
                        @php
                            $itemBalances = [];
                            // Hitung akumulasi dari ID terkecil
                            ...
                            // Balik urutan agar ID Ref besar muncul di paling atas
                            $displayData = $transactions->reverse();
                        @endphp

                        @forelse($displayData as $trx)
                        <tr class="hover:bg-orange-50">
                            <td class="px-8 py-5 text-gray-400 font-medium">#{{ str_pad($trx->id, 4, '0', STR_PAD_LEFT) }}</td>
                            <td class="px-8 py-5 text-gray-900">{{ $trx->item->name }}</td>
                            <td class="px-8 py-5 text-center">
                                <span class="bg-orange-100 text-orange-600 px-3 py-1 rounded-lg text-[9px] border border-orange-200">
                                    {{ $trx->item->location }}
                                </span>
                            </td>
                        ...
                        @empty
                        <tr><td colspan="6" class="py-24 text-center text-gray-300">Data Kosong</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
