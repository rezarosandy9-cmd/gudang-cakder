        ...
        <div class="max-w-7xl mx-auto px-4 py-3 md:py-4">
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div class="flex items-center justify-end gap-2 overflow-x-auto pb-2 md:pb-0">
                    <a href="{{ route('dashboard') }}" class="px-4 py-2.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest bg-orange-600/50 text-white border border-white/10 whitespace-nowrap">Dashboard</a>
                    <a href="{{ route('manage') }}" class="px-4 py-2.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest bg-orange-600/50 text-white border border-white/10 whitespace-nowrap">Kelola Barang</a>
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="hidden md:block">
                        @csrf 
                        <button type="submit" class="ml-2 px-5 py-2.5 bg-red-600 text-white rounded-xl text-[9px] font-black uppercase italic shadow-lg">Log Out</button>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                <form action="{{ route('report') }}" method="GET" class="relative group flex-1 sm:w-80">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama barang..." 
                           class="w-full pl-5 pr-12 py-3.5 bg-white border-2 border-orange-100 rounded-2xl focus:border-orange-500 outline-none font-bold text-gray-700 shadow-lg text-xs">
                    <button type="submit" class="absolute right-2 top-2 bg-orange-500 text-white p-2 rounded-xl">🔎</button>
                </form>
                <a href="{{ route('report.export', request()->all()) }}" class="bg-green-600 text-white px-6 py-3.5 rounded-2xl font-black uppercase italic text-[10px] shadow-xl flex items-center justify-center gap-2">
                    <span>Export Excel</span>
                </a>
            </div>
        </div>

        <div class="flex gap-2 p-1.5 bg-gray-200/50 w-fit rounded-2xl mb-8 border border-gray-200">
            <a href="{{ route('report', ['filter' => 'all']) }}" 
               class="px-6 py-2.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest transition-all {{ request('filter', 'all') == 'all' ? 'bg-white text-orange-600 shadow-md' : 'text-gray-400 hover:text-gray-600' }}">SEMUA</a>
            <a href="{{ route('report', ['filter' => 'in']) }}" 
               class="px-6 py-2.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest transition-all {{ request('filter') == 'in' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-400 hover:text-gray-600' }}">MASUK (+)</a>
            <a href="{{ route('report', ['filter' => 'out']) }}" 
               class="px-6 py-2.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest transition-all {{ request('filter') == 'out' ? 'bg-orange-500 text-white shadow-md' : 'text-gray-400 hover:text-gray-600' }}">SISA (-)</a>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left min-w-[1000px]">
                    <thead class="bg-gray-900 text-white text-[10px] font-black uppercase italic tracking-widest">
                        <tr>
                            <th class="px-8 py-6">ID Barang</th>
                            <th class="px-8 py-6">Kategori</th> <th class="px-8 py-6">Nama Barang</th>
                    ...
                    <tbody class="divide-y divide-gray-100 font-bold uppercase italic text-xs">
                        ...
                        <tr class="hover:bg-orange-50 transition-colors">
                            ...
                            <td class="px-8 py-5">
                                <span class="text-[10px] text-gray-500 font-black border-b-2 border-orange-200 pb-0.5">
                                    {{ $trx->item->category }}
                                </span>
                            </td>
